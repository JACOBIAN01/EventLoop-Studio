...
export function Panel({
  ...
  return (
    <div
      data-panel={title}
      // min-w-0 alongside min-h-0: without it, a child that refuses to wrap (e.g. Source's
      // unwrapped code lines) forces this flex item wider than its allotted share instead of
      // scrolling internally, pushing the whole panel out of alignment with its own border.
      className={`flex min-h-0 min-w-0 flex-col rounded-md border border-slate-200 bg-surface p-3 shadow-[0_1px_2px_rgba(15,23,42,0.04)] ${className ?? ''}`}
    >
