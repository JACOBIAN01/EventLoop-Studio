...
- **`setTimeout(fn, 0)` vs. `setImmediate` ordering at the very top level of a script is resolved one consistent way, and flagged as such rather than presented as a rule.** This specific race is genuinely undocumented in real Node too, and depends on process startup overhead (inside an I/O callback, `setImmediate` always and correctly wins, no ambiguity there). When it occurs, the step's caption and a dedicated hover tooltip on the Timers phase chip explain that a real terminal run can legitimately show the opposite order.
